 ...
 *
 * $Id: tal_slotted_csma.c 12332 2008-11-28 11:45:22Z uwalter $
 *
 ...
 */
#ifdef TAL_2006
    #define CONVERT_OCTETS_TO_SYM(octets)    \
                (                            \
...
#else
    #define CONVERT_OCTETS_TO_SYM(octets)   (8 * octets)
#endif
...
void slotted_csma_start(bool perform_frame_retry)
{
    ...
    else
    {
        ...
        number_of_tx_retries = aMaxFrameRetries;
    }
...
static void calculate_transaction_duration(void)
{
    ...
    {
        transaction_duration_sym = aMinLIFSPeriod;   // symbols
    }
    else
    {
        transaction_duration_sym = aMinSIFSPeriod;   // symbols
    }
    ...
    else    // next backoff boundary is within current CAP
    {
        ...
        else    // there are enough backoff periods in current CAP
        {
            ...
            if (time_after_transaction_sym < current_CAP_end_sym)
            {
                ...
                else if (timer_status == INVALID_TIMEOUT)
                {
...
void slotted_csma_state_handling(void)
{
    switch (tal_csma_state)
    {
        ...
        case CSMA_ACCESS_FAILURE:
            NB++;
            BE++;
            // ensure that BE is no more than aMacBE
            if (BE > aMaxBE)        // macMaxBE
            {
                BE = aMaxBE;        // macMaxBE
            }
            ...
            if (NB > macMaxCSMABackoffs)
            {
                ...
                tx_done(CHANNEL_ACCESS_FAILURE);
            }
            else
            {
                // restart backoff
        ...
        case TX_DONE_NO_ACK:
            if (number_of_tx_retries < aMaxFrameRetries)
            {
...
static void send_frame_at_next_backoff_boundary(void)
{
    ...
    {
        pal_get_current_time(&now_time_us);
    } while (pal_add_time_us(now_time_us, PRE_TX_DURATTION_US) <
             cca_starttime_us);
    ...
    send_frame(tal_frame_to_tx, false, false);
...
static void start_beacon_loss_timer(void)
{
    ...
    pal_timer_start(TAL_CSMA_BEACON_LOSS_TIMER,
                    timer_duration_us,
                    TIMEOUT_RELATIVE,
                    (void *)beacon_loss_timer_cb,
                    NULL);

#if (DEBUG > 0)
    if (timer_status != SUCCESS)
    {
        if (timer_status == INVALID_TIMEOUT)
        {
            ASSERT("beacon loss timer start failed: INVALID_TIMEOUT" == 0);
        }
        else if (timer_status == ALREADY_RUNNING)
        {
            ASSERT("beacon loss timer start failed: ALREADY_RUNNING" == 0);
        }
        else
        {
