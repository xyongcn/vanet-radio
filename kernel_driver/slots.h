...
1. 报文分为数据包与控制包，第一位标定
	...
	2. 1xxx：控制包
		...
		2. [1位数据或控制flag] [ 1位基础时隙申请FLAG  1位额外时隙申请FLAG] [ 5位本节点id ] [ * 时隙N信息 （1位BUSY，1位优先级，1位点到点p2p，5位节点id) ]
		3. 时隙信息包：00；基础时隙申请：10；额外时隙申请：01；
		4. 规则：
			1. 起始时先在监听中确定的的预选基础时隙发送基础时隙申请，此时不需要发送其他信息，即：1 10 [id]
			2. 时隙信息（FI）包：1 00 [id] [N条时隙信息]
			3. 额外时隙申请：
				1. 【在对应的时隙上发送额外时隙申请报文：1 01 [id]】：怎样处理公平性的问题？
				2. 【在基础时隙发送额外时隙申请】：不太好
...
#define GET_TYPE(data) 			(((data) >> 7) & 0x01)
#define GET_CONTROL_TYPE(data) 	(((data) >> 5) & 0x03)
#define GET_ID(data)			((data)&0x1f)

#define FI_GET_BUSY(data)		(((data) >> 7) & 0x01)
